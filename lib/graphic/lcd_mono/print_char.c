...
void print_char(char c, xy_t pos, const font_t * font, text_color_t color, unsigned scale)
{
    ...
    if (font_data) {
        lcd_image_bitmask(pos.x, pos.y, font->size.w, font->size.h, font_data, scale, color.inverted);
    } else {
        ...
        for (unsigned rx = 0; rx < (unsigned)font->size.w; rx++) {
            lcd_image_bitmask(pos.x + rx * use_scale, pos.y, 1, font->size.h, &pattern[rx & 1][0], use_scale, color.inverted);
        }
    }
}
