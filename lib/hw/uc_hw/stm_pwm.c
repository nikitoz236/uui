...
static inline void timer_configure_output(TIM_TypeDef * timer, unsigned ch, unsigned n, unsigned mode)
{
...
void init_pwm(const pwm_cfg_t * cfg)
{
    ...
    pwm_set_ccr(cfg, 0);
    timer_configure_output(cfg->tim, cfg->ch - 1, cfg->ch_n, 6);
