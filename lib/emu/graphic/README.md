...
- `LCD_TOP_LSB` / `LCD_TOP_MSB` — порядок битов в байте FB:
  каким битом записан верхний пиксель страницы. Зависит И от
  контроллера, И от транспорта (нормальный SPI vs USART-as-SPI,
  у которого порядок битов в байте инвертирован)

  Известные комбинации:
  - PCF8814 (Nokia 1202) по SPI — `LCD_TOP_LSB`
  - HX1230 (Nokia 1100) по USART-as-SPI — `LCD_TOP_MSB`
    (контроллер LSB top, но USART шлёт LSB-first → байт
    зеркалится по дороге → в FB хранится как MSB top)
