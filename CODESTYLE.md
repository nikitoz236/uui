...
- `#pragma once` во всех заголовочных файлах если содержат определения (структуры, типы, enum, static inline, макросы)
...
- Не ограничена
- Аргументы функций (объявления, определения, вызовы) — всегда в одну строку, не переносить
...
- Возвращаемые значения функций: `1` = успех, `0` = не успех. Тип — `unsigned`. Не использовать `int` и отрицательные коды ошибок без крайней необходимости
