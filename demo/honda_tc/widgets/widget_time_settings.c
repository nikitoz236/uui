...
{
    return size_add_padding(lcd_text_size_px(&fcfg, (xy_t){ .x = len, .y = 2 }), text_margin);
}

static void calc_pos(ctx_t * ctx, form_t * f, unsigned title_len, unsigned text_len)
{
    ctx->title_pos = align_form_pos(f, lcd_text_size_px(&fcfg, (xy_t){ .x = title_len, .y = 1 }), ALIGN_LIUI, text_margin);
    ctx->text_pos = align_form_pos(f, lcd_text_size_px(&fcfg, (xy_t){ .x = text_len, .y = 1 }), ALIGN_RIDI, text_margin);
}
